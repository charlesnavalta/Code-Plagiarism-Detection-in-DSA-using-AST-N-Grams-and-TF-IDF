...
class BinaryElement_33 {
    int itemKey;
    BinaryElement_33 ptrL;
    BinaryElement_33 ptrR;
    BinaryElement_33(int itemKey) { this.itemKey = itemKey; }
}

public class BST {
    BinaryElement_33 treeOrigin;

    public void addKey(int itemKey) {
        if (treeOrigin == null) {
            treeOrigin = new BinaryElement_33(itemKey);
            return;
        }
        BinaryElement_33 ptrWalker = treeOrigin;
        while (true) {
            if (itemKey < ptrWalker.itemKey) {
                if (ptrWalker.ptrL == null) {
                    ptrWalker.ptrL = new BinaryElement_33(itemKey);
                    break;
                }
                ptrWalker = ptrWalker.ptrL;
            } else {
                if (ptrWalker.ptrR == null) {
                    ptrWalker.ptrR = new BinaryElement_33(itemKey);
                    break;
                }
    ...
    private void depthScan(BinaryElement_33 elementPtr, List<Integer> outputBuffer) {
        if (elementPtr != null) {
