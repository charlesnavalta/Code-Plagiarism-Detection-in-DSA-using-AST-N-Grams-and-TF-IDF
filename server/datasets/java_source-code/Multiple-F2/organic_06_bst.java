...
class DictNode_5 {
    int keyVal;
    DictNode_5 leftChild, rightChild;
    DictNode_5(int k) { this.keyVal = k; }
}

public class BST {
    public DictNode_5 putTreeItem(DictNode_5 node, int val) {
        if (node == null) return new DictNode_5(val);
        if (val < node.keyVal) node.leftChild = putTreeItem(node.leftChild, val);
        else node.rightChild = putTreeItem(node.rightChild, val);
        return node;
    }

    public boolean hasTreeItem(DictNode_5 node, int val) {
        if (node == null) return false;
        if (node.keyVal == val) return true;
        return (val < node.keyVal) ? hasTreeItem(node.leftChild, val) : hasTreeItem(node.rightChild, val);
    }

    public List<Integer> dumpSorted(DictNode_5 node) {
