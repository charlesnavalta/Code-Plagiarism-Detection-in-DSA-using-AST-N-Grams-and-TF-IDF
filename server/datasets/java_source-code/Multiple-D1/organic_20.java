...
class Solution20 {
    public static int splitSegment20(int[] dataArray, int startBound, int endBound) {
    ...
    public static void performQuicksort20(int[] dataArray, int startBound, int endBound) {
        if (startBound < endBound) {
            int splitLoc = splitSegment20(dataArray, startBound, endBound);
            performQuicksort20(dataArray, startBound, splitLoc - 1);
            performQuicksort20(dataArray, splitLoc + 1, endBound);
        }
    }

    public static void main(String[] args) {
        int[] sample = {40, 10, 20, 14, 28, 42};
        performQuicksort20(sample, 0, sample.length - 1);
