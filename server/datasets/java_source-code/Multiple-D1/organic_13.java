...
class Solution13 {
    public static int splitSegment13(int[] dataArray, int startBound, int endBound) {
    ...
    public static void performQuicksort13(int[] dataArray, int startBound, int endBound) {
        if (startBound < endBound) {
            int splitLoc = splitSegment13(dataArray, startBound, endBound);
            performQuicksort13(dataArray, startBound, splitLoc - 1);
            performQuicksort13(dataArray, splitLoc + 1, endBound);
        }
    }

    public static void main(String[] args) {
        int[] sample = {41, 39, 43, 14, 28, 42};
        performQuicksort13(sample, 0, sample.length - 1);
